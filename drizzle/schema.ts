...
export const maintenanceTypeEnum = pgEnum("maintenance_type", ["lubrication", "fuel", "oil_change", "repair", "inspection", "breakdown", "other"]);
...
export const machineMaintenance = pgTable("machine_maintenance", {
  ...
  hoursAtService: integer("hours_at_maintenance"),
  notes: text("notes"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
export const machineWorkHours = pgTable("machine_work_hours", {
  ...
  startTime: timestamp("start_time").notNull(),
  endTime: timestamp("end_time"),
  hoursUsed: integer("hours_worked"),
